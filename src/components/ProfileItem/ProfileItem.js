...
const ProfileItem = ({img, name, bio, pubKey, twitter, mail, newFollowersCount, stats}) => {
        ...
        <div className="profile">
            <div className="profile-info">
                ...
                <div className="profile-info__hero">
                    ...
                    <div className="profile-info__hero-keys">
                        {mail && <a className="profile-info__hero-keys-mail">
                            {mailName === '_' ? mailName.replace('_', '') : mailName}<CheckCircle />{mailAdress}
                        </a>}
                        {twitter && <a className="profile-info__hero-keys-twitter"><Twitter />{twitter}</a>}
                        <a className="profile-info__hero-keys-key"><Key /> npub10fghh...u6m2</a>
                    </div>
                    <div className="profile-info__hero-sats">
                        {stats?.zaps_received?.msats && <p><span>{String(stats?.zaps_received?.msats).slice(0, 2)}.{String(stats?.zaps_received?.msats).slice(2, 3)}K</span> sats received</p>}
                    </div>
                </div>
            </div>
            ...
            <div className="profile-content">
                ...
                <div className="profile-content__stats">
                    <p><span>{stats.pub_following_pubkey_count}</span> Following &nbsp;&nbsp;<span>{stats.followers_pubkey_count}</span> Followers<span className="new-followers">&nbsp;+{newFollowersCount}</span></p>
                </div>
            </div>
