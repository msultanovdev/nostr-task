    ...
    try {
      ...
      for (let i = 0; i < data.profiles.length; i++) {
        const promise = axios.get(
          `https://api.nostr.band/v0/stats/profile/${data.profiles[i].pubkey}`
        );
        ...
        <Col lg={9}>
          <div>
            ...
            <div className="home-profiles">
              {fullProfiles && fullProfiles.length
                ? fullProfiles.map((profile) => {
                    const profileContent = profile[0].profile
                      ? JSON.parse(profile[0].profile.content)
                      : "";
                    return (
                      <ProfileItem
                        key={profile[0].pubkey}
